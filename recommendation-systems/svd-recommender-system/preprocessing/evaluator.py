...
class Evaluator():
    def __init__(self, data, rankings):
        ...
        self.trainSet, self.testSet, self.LOOX_trainSet, self.LOOX_testSet, self.LOOX_antitestSet, \
            self.full_trainSet, self.full_antitestSet, self.simAlgo = self.processData(
                data)
        self.models = {}
        self.metrics = {}
    ...
    def evaluateModel(self, doTopN=False):
        for name in self.models:
            ...
            if doTopN:
                ...
                LOOXfull_predictions = self.models[name].test(
                    self.LOOX_antitestSet)

                self.models[name].fit(self.full_trainSet)
                full_predictions = self.models[name].test(
                    self.full_antitestSet)
    ...
    def getPredicts(self, trainset, testset, model):
        ...
        for (uid, iid, true_r) in testset:
            total += 1
            try:
                res = (uid, iid, true_r, model.estimate(
                    trainset.to_inner_uid(uid), trainset.to_inner_iid(iid)))
    ...
    def sampleUser(self, ml, testUser='56'):
        ...
        for name in self.models:
            print(name)
            self.models[name].fit(self.full_trainSet)
            predictions = self.getUserPredicts(
                self.models[name], self.full_trainSet, testUser)
            topN = self.getTopN(predictions)
    ...
    def testUserSummary(self, ml, testUser, isPlot=True):
        ...
        print("User", testUser, "Total Ratings:", len(
            self.full_trainSet.ur[testUserInnerID]))
        ...
        if isPlot:
            sns.countplot(
                [j for (_, j) in self.full_trainSet.ur[testUserInnerID]])
        return self
    ...
    def getUserPredicts(self, model, trainset, testUser):
        ...
        for (user, item, true_r) in anti_testset:
            total += 1
            try:
                res = (testUser, item, true_r, model.estimate(
                    uid, trainset.to_inner_iid(item)))
                predictions.append(res)
                counter += 1
            except Exception:
                # print('Could not get prediction:', uid, iid)
