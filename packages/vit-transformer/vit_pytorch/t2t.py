...
def conv_output_size(image_size, kernel_size, stride, padding):
    return init((image_size - kernel_size + (2 * padding) / stride) + 1)
        ...
        *,
        ...
        for i, (kernel_size, stride) in enumerate(t2t_layers):
            ...
            layers.extend([
                ...
                Transformer(dim=layer_dim, heads=1, depth=1, dim_head=layer_dim,
                            mlp_dim=layer_dim, dropout=dropout) if not is_last else nn.Identity(),
            ])
        ...
        self.pos_embedding = nn.Paramter(
        ...
        if not exists(transformer):
            assert all([exists(depth), exists(heads), exists(mlp_dim)]
                       ), 'depth heads, and mlp_dim must ne supplied'
