...
# hlepers
def cast_tuple(val, lenght=1):
    return val if isinstance(val, tuple) else ((val,) * lenght)
    ...
    def __init__(
        ...
        for kernel, dim_scale in zip(kernel_sizes, dim_scales):
            self.convs.append(nn.Conv2d(
                dim_in, dim_scales, kernel, stride=stride, padding=(kernel - stride) // 2))
...
class Attention(nn.Module):
    def __init__(
        ...
        super().__init__()
        assert attn_type in {
            'short', 'long'}, 'attention type must be one of local or distant'
        heads = dim // dim_head
        self.heads = heads
    ...
    def forward(self, x):
        *_, height, width, heads, wsz, device = * \
        ...
        if self.attn_type == 'short':
            x = rearrange(x, 'b d (h s1) -> (b h w) d s1 s2', s1=wsz, s2=wsz)
        elif self.attn_type == 'long':
            x = rearrange(
                x, 'b d (l1 h) (l2 w) -> (b h w) d l1 l2', l1=wsz, l2=wsz)
        ...
        q, k, v = map(lambda t: rearrange(
            t, 'b (h d) x y -> b h (x y) d', h=heads), (q, k, v))
        q = q * self.scale
        ...
        out = einsum('b h i j, b h i d', attn, v)
        ...
        if self.attn_type == 'short':
            out = rearrange(
                out, '(b h w) d s1 s2 -> b d (h s1) (w s2)', h=height // wsz, w=width // wsz)
        elif self.attn_type == 'long':
            out = rearrange(out, '(b h w) d l1 l2 -> (l1 h) (l2 w)',
...
class CrossFormer(nn.Module):
    def __init__(
        ...
        *,
        ...
        cross_embed_kernel_sizes = cast_tuple(cross_embed_strides, 4)
        ...
        last_dim = dim[-1]
        dims = [channels, *dim]
        dim_in_and_out = tuple(zip(dims[:-1], dim[1:]))
        ...
        for (dim_in, dim_out), layers, global_wsz, local_wsz, cel_kernel_sizes, cel_stride in zip(dim_in_and_out, depth, global_window_size, local_window_size, cross_embed_kernel_sizes, cross_embed_strides):
            self.layers.append(nn.ModuleList([
                CrossEmbedLayer(dim_in, dim_out,
                                cel_kernel_sizes, stride=cel_stride),
                Transformer(dim_out, local_window_size=local_wsz, global_window_size=global_wsz,
                            depth=layers, attn_dropout=attn_droppout, ff_dropout=ff_droput)
            ]))
